...
def check_filing_freshness(
    doc_date_str: str,
    ticker: Optional[str] = None,
    threshold_months: int = FILING_STALENESS_MONTHS
) -> Tuple[bool, Optional[str]]:
    """
    Evaluates whether a filing date exceeds the staleness threshold.
    Returns: (is_stale, warning_message)
    """
    if not doc_date_str:
        return True, "Filing Freshness Warning: Filing date missing or unparseable; treated as unverified freshness."

    try:
        doc_date = datetime.strptime(doc_date_str.strip(), "%Y-%m-%d").date()
    except Exception:
        return True, f"Filing Freshness Warning: Invalid date format '{doc_date_str}'; expected YYYY-MM-DD."
    ...
    if age_months > threshold_months:
        ...
        warning = (
            f"Filing Freshness Warning{prefix}: The latest available filing is dated {doc_date_str} "
